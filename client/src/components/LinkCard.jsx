...
const platformMap = {
  github: { icon: GitHubLogo, color: '#c9d1d9' },
  linkedin: { icon: LinkedInLogo, color: '#0a66c2' },
  x: { icon: TwitterLogo, color: '#e7e9ea' },
  instagram: { icon: InstagramLogo, color: '#e4405f' },
  youtube: { icon: YouTubeLogo, color: '#ff0033' },
  website: { icon: WebsiteLogo, color: '#ffffff' },
  email: { icon: EmailLogo, color: '#ffffff' },
};

export default function LinkCard({ link, index }) {
  const platform = platformMap[link.platform] || { icon: WebsiteLogo, color: '#ffffff' };
  ...
  return (
    <motion.div
      ...
      transition={{ delay: 0.08 * (index || 0), duration: 0.4, ease: 'easeOut' }}
      className="w-full"
    >
      <MagicCard
      ...
      >
        ...
        >
          <div className="shrink-0 w-9 h-9 rounded-lg bg-gradient-to-b from-[#202020] to-[#191919] shadow-[0_0.5px_0_#ffffff50,0_2px_6px_#00000090_inset] flex items-center justify-center">
            <Icon className="w-4 h-4 transition-colors duration-150" style={{ color: platform.color }} />
          </div>
